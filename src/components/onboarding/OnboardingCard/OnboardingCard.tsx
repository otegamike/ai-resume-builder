...
const ResumeSelector = dynamic(() => import("@/components/resume/ResumeSelector"), {
  ssr: false,
  loading: () => <div className={styles.loadingPlaceholder}>Loading uploader...</div>,
});
...
export default function OnboardingCard({
  ...
  const [showSkip, setShowSkip] = useState(skipDelayMs === 0 || skipDelayMs == null);
  ...
  useEffect(() => {
    if (skipDelayMs === 0 || skipDelayMs == null) {
      setShowSkip(true);
      return;
    }
    setShowSkip(false);
    const t = setTimeout(() => setShowSkip(true), skipDelayMs);
  ...
  const totalSteps = includeResumeStep ? 4 : 3;
  ...
  const canProceed = () => {
    switch (step) {
      ...
      case 2:
        if (!includeResumeStep) return true;
        return true;
      case 3:
        return mode !== null;
      default:
  ...
  const handleCompactComplete = async () => {
    setSubmitting(true);
    setError("");
    try {
  ...
  const cardClass = `${styles.onboardingCard} ${variant === "compact" ? styles.compact : ""}`;
  ...
  return (
    <div className={cardClass}>
      ...
      <div className={styles.stepsWrapper}>
        ...
        <div className={`${styles.stepContent} ${step === 0 ? styles.stepActive : ""}`}>
          <div className={styles.stepHeader}>
            ...
            <p className={styles.stepSubtitle}>Tell us a bit about yourself so we can personalise your experience.</p>
          </div>
        ...
        <div className={`${styles.stepContent} ${step === 1 ? styles.stepActive : ""}`}>
          <div className={styles.stepHeader}>
            ...
            <p className={styles.stepSubtitle}>Select all that apply. You can use all of these later.</p>
          </div>
          <div className={styles.goalsGrid}>
            {GOALS.map((goal) => {
              ...
              return (
                ...
                >
                  <div className={styles.goalCardCheck}>{selected && <Check className={styles.goalCardCheckIcon} />}</div>
        ...
        <div className={`${styles.stepContent} ${step === 2 ? styles.stepActive : ""}`}>
          <div className={styles.stepHeader}>
            <div className={styles.stepBadge}>Step {includeResumeStep ? 3 : 3}</div>
        ...
        {/* Step 3: Existing Resume - only when includeResumeStep */}
        {includeResumeStep && (
          <div className={`${styles.stepContent} ${step === 3 ? styles.stepActive : ""}`}>
            <div className={styles.stepHeader}>
              ...
              <p className={styles.stepSubtitle}>Upload an existing resume and we&apos;ll extract your info, or start fresh.</p>
            </div>

            {mode === null && (
              <div className={styles.resumeOptions}>
                <button type="button" className={styles.resumeOptionCard} onClick={() => setMode("upload")}>
                  ...
                  <span className={styles.resumeOptionDesc}>
                    We&apos;ll extract your info so you can pick up where you left off.
                  </span>
                  <ArrowRight className={styles.resumeOptionArrow} />
                </button>
                <button type="button" className={styles.resumeOptionCard} onClick={() => setMode("scratch")}>
                  <div className={styles.resumeOptionIconWrapper}>
      ...
      <div className={styles.navButtons}>
        {step > 0 && (
          <button type="button" className={styles.btnBack} onClick={handleBack} disabled={submitting}>
        ...
        {/* Compact complete (no resume step) */}
        {!includeResumeStep && step === totalSteps - 1 && (
          <button type="button" className={styles.btnPrimary} onClick={handleCompactComplete} disabled={submitting}>
            {submitting ? (
              <>
              ...
              <>
                Complete profile <ArrowRight className={styles.btnIcon} />
              </>
            )}
          </button>
        )}

        {includeResumeStep && step === totalSteps - 1 && mode === "scratch" && (
          <button type="button" className={styles.btnPrimary} onClick={handleStartFromScratch} disabled={submitting}>
            {submitting ? (
              <>
                <Loader2 className={styles.btnSpinner} /> Saving...
              </>
        ...
        {includeResumeStep && step === totalSteps - 1 && mode === "upload" && (
          <button
            type="button"
