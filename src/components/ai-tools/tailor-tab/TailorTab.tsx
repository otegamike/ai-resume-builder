...
import styles from "./TailorTab.module.css";
...
export default function TailorTab() {
  ...
  async function createTailoredResume() {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) throw new Error(data.error || "Failed to save tailored resume");
  ...
  return (
    <>
      ...
      <div className={styles.splitGrid}>
          ...
          <form className={styles.form} onSubmit={handleTailorSubmit}>
            ...
            <div className={styles.metadataRow}>
              <div className={styles.field}>
                ...
                <input id="targetTitle" type="text" placeholder="e.g. Senior Frontend Engineer" value={targetTitle} onChange={(e) => setTargetTitle(e.target.value)} className={styles.input} disabled={isBusy} />
              </div>
              <div className={styles.field}>
                <label htmlFor="targetCompany" className={styles.label}>
                  Company Name <small className={styles.optionalText}>(optional)</small>
                </label>
                <input id="targetCompany" type="text" placeholder="e.g. Google" value={targetCompany} onChange={(e) => setTargetCompany(e.target.value)} className={styles.input} disabled={isBusy} />
              </div>
            </div>
            <JobDescriptionInput job={{ ...job, handleJobImageChange, handleJobModeChange }} disabled={isBusy} textareaId="jobText" />
        ...
        <section className={styles.resultsPanel} id="tailorResultPanel">
          {report ? (
