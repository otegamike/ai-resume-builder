...
description: >
  Use this skill whenever generating a new HTML resume template for AgenticApp.cv.
  Triggers include: "create a new template", "add a resume template", "design a CV template",
  "build templateN", or any request to produce a .html file that fits the AgenticApp.cv template pipeline.
...
# AgenticApp.cv Template Generator Skill

You are generating a professional, ATS-optimized, visually polished HTML resume template
for the **AgenticApp.cv** resume builder. Read every section of this skill before writing code.
...
## 2. Fixed Page Geometry ΓÇö NEVER Change These
...
<!-- MANDATORY SCALING + PAGINATION SCRIPT ΓÇö copy verbatim, change nothing -->
<script>
  const CV_WIDTH  = 794;
...
- Copy the script block **verbatim**. Do not rename functions, change constants, or remove the
  `postMessage` call ΓÇö the parent app depends on it.
...
- `document.fonts.ready.then(scaleCv)` must be the final trigger ΓÇö Google Fonts load async.
...
```html
{{#experience}}
  <div>{{company}} ΓÇö {{role}}</div>
  <div>{{startDate}} ΓÇô {{endDate}}</div>
...
experience[]
  ...
  .startDate     string
  .endDate       string
  .description   string[]   ΓåÉ iterate with {{#description}}{{.}}{{/description}}
...
skills[]         string[]   ΓåÉ iterate with {{#skills}}{{.}}{{/skills}}
skillCategories[]
  .id            string
  .category      string
  .skills        string[]   ΓåÉ iterate with {{#skills}}{{.}}{{/skills}}
...
structures. Each pattern has been validated against the 794├ù1123 page geometry.

### Pattern A ΓÇö Two-Column (Sidebar + Main)
```css
...
### Pattern B ΓÇö Header + Two-Column Body
```css
.cv { display: flex; flex-direction: column; }
.header { /* full-width accent header, min-height 160ΓÇô220px */ }
...
### Pattern C ΓÇö Full-Width Single Column (Classic/ATS-first)
```css
...
Best for pure ATS parsing ΓÇö no background colors in main content area.

### Pattern D ΓÇö Accent-Left Geometric
```css
...
- `Playfair Display` ΓÇö editorial, elegant serif
- `Cormorant Garamond` ΓÇö refined, literary serif
- `Josefin Sans` ΓÇö geometric, architectural
- `Outfit` ΓÇö modern geometric, approachable
- `DM Serif Display` ΓÇö contemporary editorial
- `Fraunces` ΓÇö optical, expressive serif
- `Syne` ΓÇö contemporary grotesque with personality
- `Gilda Display` ΓÇö classic roman with subtle flair

**Approved body fonts** (highly readable at small sizes):
- `Raleway` ΓÇö elegant, thin weights available
- `Source Sans 3` ΓÇö neutral, highly legible
- `Barlow` ΓÇö compact, great for dense layouts
- `Lato` ΓÇö clean, warm sans-serif
- `Nunito Sans` ΓÇö rounded, friendly
- `IBM Plex Sans` ΓÇö technical, precise
- `Mulish` ΓÇö clean, modern
- `DM Sans` ΓÇö geometric, pairs with DM Serif

**Never use**: Arial, Helvetica, Times New Roman, Comic Sans, Courier, Impact, or any font
not available on Google Fonts. Avoid Inter and Roboto ΓÇö they produce a generic AI look.

### Type Scale (all in rem, base 20px = 1rem)
```
Candidate name:     1.8rem ΓÇô 2.4rem, font-weight 700ΓÇô800
Job title:          0.75rem ΓÇô 0.9rem, letter-spacing 3ΓÇô5px, uppercase, weight 300ΓÇô400
Section headers:    0.65rem ΓÇô 0.85rem, letter-spacing 3ΓÇô4px, uppercase, weight 600ΓÇô700
Entry titles:       0.78rem ΓÇô 0.88rem, weight 600ΓÇô700
Entry subtitles:    0.72rem ΓÇô 0.78rem, weight 400, color muted
Body / descriptions:0.70rem ΓÇô 0.78rem, weight 300ΓÇô400, line-height 1.6ΓÇô1.8
Contact items:      0.72rem ΓÇô 0.78rem
```

Never go below `0.68rem` ΓÇö text becomes illegible when scaled for A4 export.
...
These are starting points ΓÇö vary them to produce a unique template:
...
- Never use pure `#000000` for text ΓÇö use `#1a1a1a` or `#2a2a2a`
- Never use bright/neon colors ΓÇö they destroy ATS scan aesthetics and look unprofessional
...
### Structure rules
1. **Name must be a plain text element** ΓÇö never an SVG, image, or canvas.
...
3. **Reading order** must match logical resume order: name ΓåÆ title ΓåÆ summary ΓåÆ experience
   ΓåÆ education ΓåÆ skills. Achieved automatically when using standard HTML flow order.
4. **No text in `background-image`** ΓÇö decorative shapes must use `background-color`,
...
6. **Description bullet points** (`{{.}}` items from `description[]`) must be real `<p>`
   or `<li>` elements ΓÇö never rendered as border/pseudo-element tricks.
...
**Pattern 1 ΓÇö Colored label with bottom border**
```css
.section-label {
  font-size: 0.68rem;
  letter-spacing: 4px;
...
**Pattern 2 ΓÇö Inline rule (horizontal line extends after text)**
```css
.section-label {
  display: flex;
  align-items: center;
...
**Pattern 3 ΓÇö Double rule (balanced line, label centered between two rules)**
```css
.section-label {
  display: flex;
  align-items: center;
...
<div class="exp-entry">
  <div class="exp-header">
    ...
    <span class="exp-dates">{{startDate}} ΓÇô {{endDate}}</span>
  </div>
...
.exp-desc::before { content: 'ΓÇô'; position: absolute; left: 0; color: var(--accent); }
```

### Skill category name (for categorized skills)
Always define an explicit `color` ΓÇö these often appear on tinted or dark backgrounds where inheritance doesn't provide sufficient contrast:
```css
.skill-category-name {
  font-weight: 700;
  font-size: 0.9rem;
...
**PDF export behavior:** During PDF export, the `groupBlockParentHeaders()` function in `exportUtils.ts` wraps the section label + first `.block` child inside each `block__parent` into a `<div class="block" style="display:contents; break-inside:avoid">`. This ensures the label and first item stay together on the same page in the PDF. Design accordingly ΓÇö avoid relying on the section label being a direct first-child of `block__parent` in CSS selectors (prefer `.block__parent > .block` over `.block__parent > :first-child`).
...
## 11. Decorative Elements ΓÇö Safe Techniques
...
### Things that DO NOT export correctly ΓÇö avoid these
...
- `color-mix()` ΓÇö html2canvas cannot parse this function; use hardcoded `rgba()` or `opacity` instead
...
**Visual balance**
- [ ] Left column width 220ΓÇô260px (two-column layouts)
- [ ] Right column padding ΓëÑ 24px all sides
- [ ] Section label margin-bottom ΓëÑ 10px
- [ ] Minimum contrast: accent color vs white ΓëÑ 4.5:1

---

## 13. Design Differentiation ΓÇö Make It Memorable
...
**Do not produce a template that looks like template1ΓÇô4.**  
...
<style>
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .bullet-list li::before { content: 'ΓÇó'; position: absolute; left: 0; color: var(--accent); }
  ...
  .exp-desc { font-size: 0.72rem; line-height: 1.7; color: var(--text); padding-left: 14px; position: relative; }
  .exp-desc::before { content: 'ΓÇô'; position: absolute; left: 0; color: var(--accent); font-weight: 700; }
      ...
      <div class="right">
        ...
        <div class="block">
          ...
          <div class="edu-entry">
            ...
            <div class="edu-detail">{{school}} ┬╖ {{startDate}} ΓÇô {{endDate}}</div>
          </div>
          {{/education}}
        </div>
        {{/education}}
